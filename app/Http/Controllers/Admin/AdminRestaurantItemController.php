...
class AdminRestaurantItemController extends Controller
{
    public function store_items()
    {
        return view('admin.items.itemList');
    }


    public function store_items_get_all(Request $request)
    {
        $all_items = restaurant_item::select('id', 'restaurant_id', 'main_category_id', 'sub_category_id', 'item_name', 'item_price', 'item_image', 'created_at')->get();
        return DataTables::of($all_items)
            ->addColumn('action', function ($all_items) {
                return ' <a href="' . route('admin.store.items.edit', $all_items->id) . '"> <button class="btn btn-success btn-info btn-sm"><i class="fas fa-edit"></i> </button></a>
                        <button id="' . $all_items->id . '" onclick="deletestoreitem(this.id)" class="btn btn-danger btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#deletstoreitem"><i class="far fa-trash-alt"></i> </button>';
            })
            ->editColumn('created_at', function ($all_items) {
                return Carbon::parse($all_items->created_at)->format('d-F-Y');
            })
            ->editColumn('restaurant_id', function ($all_items) {
                $type = Restaurant::select('restaurant_name')->where('id', $all_items->restaurant_id)->first();
                if ($type) {
    ...
    public function store_item_create()
    {
        $all_res = Restaurant::select('id', 'restaurant_name')->get();
        $main_cats = main_category::select('id', 'main_category_name')->get();
        $sub_cats = sub_category::select('id', 'sub_category_name')->get();
        $meas = measurement::all();
        return view('admin.items.itemCreate', compact('all_res', 'main_cats', 'sub_cats', 'meas'));
    }

    public function store_item_save(Request $request)
    {
    ...
    public function store_item_edit($id)
    {
        $item_edit = restaurant_item::where('id', $id)->first();
        $all_res = Restaurant::select('id', 'restaurant_name')->get();
        $main_cats = main_category::select('id', 'main_category_name')->get();
        $sub_cats = sub_category::select('id', 'sub_category_name')->get();
        $meas = measurement::all();
        return view('admin.items.itemEdit', compact('item_edit', 'all_res', 'main_cats', 'sub_cats', 'meas'));
    }
    ...
    public function store_item_update(Request $request)
    {
        ...
        $update_item->restaurant_id = $request->restaurant_id;
    ...
    public function store_item_delete(Request $request)
    {
        ...
        return back()->with('success', 'Store Item Successfully Deleted');
    }
